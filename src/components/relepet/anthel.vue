<template>
    <section class="DiologBox">
        ...
        <!--疫苗名称组件传来两个值-->
        <anthName v-if="showName" v-on:childValue="childValue"></anthName>
        <div @click="click" ref="ons"></div> 
    </section>
</template>

<script>
import anthName from '@/components/relepet/anthName'
export default {
    name:'Anthel',
    components:{
        anthName
    ...
    methods:{
        ...
        click(){
           this.$emit('anthelValue',this.numx.num,this.showName,this.version,this.timeIn) 
        }
    },
    computed:{
    },
